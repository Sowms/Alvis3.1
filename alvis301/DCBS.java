...
public class DCBS extends Algorithm {
	...
	public void run() {
		...
		while (!bigopen.isEmpty()) {
			...
			/*
			 * for (Node x : boundary) { x.setState(alvis3.iitm.ac.in.State.deleted); }
			 */
			display();
			boundary2.clear();
			for (ArrayList<Node> x : oneLevel) {
				...
				if (k <= bw) {
					best.add(x);
					// x.get(1).setState(alvis3.iitm.ac.in.State.boundary);
	 ...
	 * 
	 * Node node = nodePair.get(0); node.setState(alvis3.iitm.ac.in.State.path);
	 * updateNode(node); Node parent = nodePair.get(1); Node child = node; while
	 * (parent != null) { parent.setState(alvis3.iitm.ac.in.State.path); ArrayList<Edge>
	 ...
	 * e.setState(alvis3.iitm.ac.in.State.path); updateEdge(e); break; } }
